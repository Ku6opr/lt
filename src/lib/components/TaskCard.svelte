<script>
  ...
  import { accentSplit, simplifyAccent } from '../engine/stem.js';

  $: L = UI[$lang];
  $: acc = task && task.targetFormA ? accentSplit(simplifyAccent(task.targetFormA), task.stem.length) : null;
  $: pA = task && task.promptA ? simplifyAccent(task.promptA) : null;
  $: pAcc = pA && task.prompt && task.prompt.text == null ? accentSplit(pA, task.prompt.stem.length) : null;
  $: lA = task && task.leadA ? simplifyAccent(task.leadA) : null;
  $: tA = task && task.trailA ? simplifyAccent(task.trailA) : null;
  ...
  {#if hasTask}
    <div style="text-align:center;margin:0 0 clamp(16px,3.5cqw,28px)">
      <div style="font-family:var(--font-heading);font-size:clamp(38px,8cqw,64px);line-height:1.02">
        {#if task.prompt.text}{pA || task.prompt.text}{:else if pAcc}<WordForm stem={pAcc[0]} tail={pAcc[1]} />{:else}<WordForm stem={task.prompt.stem} tail={task.prompt.tail} />{/if}
      </div>
    ...
    <label style="display:flex;justify-content:center;margin-bottom:var(--space-3);cursor:text;padding:6px 0">
      <div style="display:inline-flex;align-items:baseline;gap:8px;font-family:var(--font-heading);font-size:clamp(24px,4.5cqw,34px)">
        {#if task.hasLead}<span class="text-muted">{lA || task.lead}</span>{/if}
        ...
        {#if task.trail}<span style="color:var(--color-text)">{tA || task.trail}</span>{/if}
      </div>
    </label>
    ...
    <div style="min-height:clamp(124px,18cqw,152px);border-top:1px solid var(--color-divider);margin-top:var(--space-3);padding-top:var(--space-4)">
      {#if revealed}
        <div style="text-align:center">
          <div class="card-kicker">{L.correctForm}{#if correctMark}<span style="color:#4a9d5b;margin-left:6px" title={L.matchedYours}>✓</span>{/if}</div>
          <div style="font-family:var(--font-heading);font-size:clamp(30px,6cqw,50px);line-height:1.02;margin:6px 0 12px">
            {#if task.hasLead}<span style="color:var(--color-neutral-500);font-size:.68em">{lA || task.lead} </span>{/if}{#if acc}<WordForm stem={acc[0]} tail={acc[1]} />{:else}<WordForm stem={task.stem} tail={task.tail} />{/if}{#if task.trail}<span style="margin-left:0.3em">{tA || task.trail}</span>{/if}
          </div>
